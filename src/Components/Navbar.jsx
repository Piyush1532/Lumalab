...
className='flex justify-between items-center px-4 sm:px-12 lg:px-24 xl:px-24 py-4 sticky top-0 z-20 backdrop-blur-xl font-medium bg-white/50 dark:bg-gray-900/70' >
...
{sideBarOpen && (
  <div
    className='fixed top-0 left-0 w-full h-screen bg-black/40 z-1 sm:hidden'
    onClick={() => setSideBarOpen(false)}
  />
)}

      <div className={`text-gray-700 dark:text-white sm:text-sm ${!sideBarOpen ? 'max-sm:w-0 overflow-hidden' : 'max-sm:w-60 max-sm:pl-10'} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-[#6200de] max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5   transition-all duration-300 z-10`}>
